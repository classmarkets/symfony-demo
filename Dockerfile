...
# For instance:
#
#     RUN apk add --update --no-cache libmcrypt-dev && docker-php-ext-install mcrypt
#
...
# webroot location, listening port of the php-fpm process, fastcgi_params, etc.
...
# (remember that we skipped script handlers and autoloader generation earlier).
#
# This is also the time to run any other build steps, such as compiling assets.
#
